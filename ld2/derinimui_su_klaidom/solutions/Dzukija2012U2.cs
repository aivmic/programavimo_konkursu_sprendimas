...
public class Dzukija2012U2
{
    ...
    public Output Run(Input input)
    {
        ...
        for (int y = 0; y < input.k; y++)
        {
            ...
            {
                if (i + 1 == input.ribaa[y])
                {
                    atsakymas[i] = input.korteles[input.ribab[y] - 1];
                    laikinas = atsakymas[i];
                }
                if (i + 1 == input.ribab[y])
                {
                    atsakymas[i] = input.korteles[input.ribaa[y] - 1];
                    input.korteles[i] = atsakymas[i];
                    input.korteles[input.ribaa[y]-1] = laikinas;
                }
                else
                {
                    atsakymas[y] = input.korteles[i];
